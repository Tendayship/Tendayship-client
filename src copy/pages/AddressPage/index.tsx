...
const AddressPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 간단한 유효성 검사
    if (Object.values(addressInfo).some(value => !(value as string).trim())) {
      alert("모든 정보를 입력해주세요.");
      return;
    }
    ...
    <>
      <div className="bg-gray-100 min-h-screen flex flex-col">
        ...
        <div className="flex flex-col items-center mt-20">
          ...
          <main className="bg-white p-10 mt-10 rounded-lg shadow-lg w-[500px] text-center">
            <h1 className="text-3xl font-bold mb-2">주소</h1>
            <p className="text-gray-600 mb-8">배송 받으실 주소를 입력해 주세요</p>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col items-center space-y-6">
              ...
              <div className="w-full text-left">
                <label htmlFor="postcode" className="block text-sm font-medium text-gray-700 mb-1">우편 번호</label>
                <div className="flex space-x-2">
                  <input
                    ...
                    className="flex-1 h-12 px-4 border border-gray-300 rounded-md"
                    readOnly
                  />
                  <button
                    type="button"
                    onClick={() => setIsPostcodeModalOpen(true)} // ◀️ 클릭 시 모달 열기
                    className="bg-[#709ECD] text-white h-12 px-4 rounded-md hover:bg-[#5a8ac4]"
                  >
                    우편 번호 찾기
                  </button>
                </div>
              ...
              <button
                type="submit"
                className="w-full h-12 mt-6 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition-colors disabled:bg-gray-400"
                disabled={isLoading}
              >
                {isLoading ? '등록 중...' : '등록'}
              </button>
      ...
      {isPostcodeModalOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 100 }}>
          <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', backgroundColor: 'white', padding: '20px' }}>
            ...
            <button onClick={() => setIsPostcodeModalOpen(false)} style={{ marginTop: '10px' }}>닫기</button>
          </div>
        </div>
      )}
    </>
