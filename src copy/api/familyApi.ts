 ...
 */
export const createFamilyGroup = async (payload: CreateGroupPayload): Promise<FamilyGroup> => {
  // 매트릭스: POST /family/create
  const response = await axiosInstance.post<FamilyGroup>('/family/create', payload);
 ...
 */
export const joinGroupByCode = async (payload: JoinGroupPayload): Promise<JoinedGroupInfo> => {
  // 매트릭스: POST /family/join (이 엔드포인트가 코드 검증 및 가입을 모두 처리한다고 가정)
  // 실제 백엔드 구현에 따라 엔드포인트나 payload가 달라질 수 있습니다.
  const response = await axiosInstance.post<JoinedGroupInfo>('/family/join', payload);
  return response.data;
}
 ...
 */
export const registerRecipient = async (groupId: string, payload: RecipientPayload): Promise<void> => {
